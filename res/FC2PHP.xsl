...
<xsl:template name="addParentDescription"> 
	...
	<xsl:for-each select="S100FC:featureBinding">
		* @property <xsl:value-of select="S100FC:association/@ref"/>_<xsl:value-of select="S100FC:role/@ref"/>[<xsl:value-of select="S100FC:multiplicity/S100Base:lower"/>..<xsl:value-of select="S100FC:multiplicity/S100Base:upper"/><xsl:if test="S100FC:multiplicity/S100Base:upper/@infinite='true'">*</xsl:if>] <xsl:value-of select="S100FC:informationType/@ref"/><xsl:value-of select="S100FC:featureType/@ref"/> |Defined in <xsl:value-of select="$currCode"/>
	</xsl:for-each>
