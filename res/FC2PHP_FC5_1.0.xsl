...
xsi:schemaLocation="http://www.iho.int/S100FC"
...
xmlns:xlink="http://www.w3.org/1999/xlink" 
xmlns:S100FD="http://www.iho.int/S100FD" 
-->
	...
	namespace fiho\s100\<xsl:value-of select="translate(S100FC:productId, '-', '')"/>;
	...
	define ( 'CURRENT_PS', '<xsl:value-of select="translate(S100FC:productId, '-', '')"/>' );
	/**
	...
	*/
	...
	<xsl:for-each select="S100FC:S100_FC_SimpleAttributes/S100FC:S100_FC_SimpleAttribute">
			...
			*/
			...
			{
				public $valueType ='<xsl:value-of select="S100FC:valueType"/>';
			...
			<xsl:if test="not(S100FC:superType)">AbstractFeatureType</xsl:if>
		{
			...
			*/
			public function __construct()
			{
				...
				<!--
				//Information and Feature bindings
				
				<xsl:for-each select="S100FC:featureBinding | S100FC:informationBinding">
					<xsl:if test="S100FC:featureType/@ref">	$this->addFeatureBinding('<xsl:value-of select="S100FC:association/@ref"/>','<xsl:value-of select="S100FC:role/@ref"/>', '<xsl:value-of select="S100FC:featureType/@ref"/>', <xsl:value-of select="S100FC:multiplicity/S100Base:lower"/>, <xsl:value-of select="S100FC:multiplicity/S100Base:upper"/>
						<xsl:if test="S100FC:multiplicity/S100Base:upper/@infinite='true'">MAX_OCCUR</xsl:if> );
					</xsl:if>
					
					<xsl:if test="S100FC:informationType/@ref"> $this->addInformationBinding('<xsl:value-of select="S100FC:association/@ref"/>', '<xsl:value-of select="S100FC:role/@ref"/>', '<xsl:value-of select="S100FC:informationType/@ref"/>', <xsl:value-of select="S100FC:multiplicity/S100Base:lower"/>, <xsl:value-of select="S100FC:multiplicity/S100Base:upper"/>
						<xsl:if test="S100FC:multiplicity/S100Base:upper/@infinite='true'">MAX_OCCUR</xsl:if>);
					</xsl:if>
				</xsl:for-each>
				-->
